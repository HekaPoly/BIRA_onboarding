...
- [Connaissances de base de la ligne de commande](https://linuxcommand.org/lc3_learning_the_shell.php)
- Un compte [GitHub](https://github.com/)

### Qu'est-ce que Git ?

Git est un système de contrôle de version distribué qui suit les modifications dans les fichiers et coordonne le travail entre plusieurs développeurs. Il vous permet de sauvegarder des versions de votre projet à différents moments et de collaborer efficacement avec d'autres.

### Qu'est-ce qu'un commit ?

Un **commit** est l'élément fondamental de Git. C'est une version de votre projet à un instant donné.

**Caractéristiques d'un commit :**
- **Version complète** : Chaque commit contient l'état de tous les fichiers de votre projet
...
```
Répertoire de travail → [Ajout de fichiers (git add)] → Zone de staging → [Création d'un commit (git commit)] → Dépôt local
```
...
![Git branches](https://us1.discourse-cdn.com/inductiveautomation/original/3X/0/d/0d19837bbaf82a71ab35b2a62c5f8776af8e900a.png)
...
**Rappel :** Un commit est une version permanente de votre projet. Avant de créer ce commit, vous devez choisir quelles modifications inclure (c'est le processus de "staging" avec `git add`).
...
1. Supprimez le fichier `fichier_a_supprimer.txt` que vous avez créé en exécutant la commande suivante:
```bash
...
Remarquez que le fichier est désormais marqué comme « supprimé » et sera retiré de l’historique lors du prochain commit.
...
git restore --staged membres.txt
git restore membres.txt
```

2. Restaurer le fichier fichier_a_supprimer.txt en exécutant la commande suivante:
```bash
# Restaurer le fichier fichier_a_supprimer.txt à son dernier état committé
git restore --staged fichier_a_supprimer.txt
git restore fichier_a_supprimer.txt
```
...
Observez que les modifications apportés aux fichiers `membres.txt` et `fichier_a_supprimer.txt` ont été annulées!
...
Crée une version de vos modifications stagées.
...
3. Crééz votre première branche en exécutant la commande suivante:
```bash
# Créer une branche
git branch ma_premiere_branche
```

4. Basculez sur la branche `mon_premier_commit` en exécutant la commande suivante:
```bash
# Basculer sur main
git checkout main
```
...
2. **Compare branch** : `votre_nom` (votre branche)
...
Lorsque tout ceci est fait, vous pouvez créer votre pull request en cliquant sur `Create pull request`.
